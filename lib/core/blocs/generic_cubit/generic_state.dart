...
class GenericDimissLoadingState<T> extends GenericCubitState<T> {
  const GenericDimissLoadingState({required super.data, required super.changed});
...
class GenericConnectionError<T> extends GenericCubitState<T> {
  const GenericConnectionError({required super.data, required super.changed});
...
class GenericErrorState<T> extends GenericCubitState<T> {
  const GenericErrorState(
      {required data, required Failure super.responseError, required super.changed})
      : super(data: data);
  @override
  List<Object> get props => [changed];
}
